...
for (group in 1:nrow(groups)) {
  ...
  for (i in 1:plotrows) {
    ...
    rect(xleft=rgyw-0.5,ybottom=-1,xright=rgyw+3.5,ytop=ymax,col="LemonChiffon",border=F)
    rect(xleft=agct-0.5,ybottom=-1,xright=agct+3.5,ytop=ymax,col="LightGoldenrod",border=F)
    ...
    if (length(cloneIDs) > 0) {
      ...
      segments(x0=group_dels$Pos-0.5,y0=group_dels$y,x1=group_dels$End+0.5,col=basecolors[5],lwd=1)
    } else {
  ...
  for (i in 1:plotrows) {
    ...
    rect(xleft=rgyw-0.5,ybottom=refy,xright=rgyw+3.5,ytop=ymax,col=rgb(254,227,145,max=255),border=F)
    rect(xleft=agct-0.5,ybottom=refy,xright=agct+3.5,ytop=ymax,col=rgb(254,196,79,max=255),border=F)
    grid(col=grey(0.5))
